...
export default {
	...
	getters: {
			...
			return state.CVAll[JSON.stringify(params)] ?? {}
				...
				getCvByCosmosAddress: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.CvByCosmosAddress[JSON.stringify(params)] ?? {}
		},
	...
	actions: {
		...
		async QueryCvByCosmosAddress({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryCvByCosmosAddress(query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryCvByCosmosAddress({...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
				commit('QUERY', { query: 'CvByCosmosAddress', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryCvByCosmosAddress', payload: { options: { all }, params: {...key},query }})
				return getters['getCvByCosmosAddress']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryCvByCosmosAddress API Node Unavailable. Could not perform query: ' + e.message)
