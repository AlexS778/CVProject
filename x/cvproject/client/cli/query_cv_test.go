...
func networkWithCVObjects(t *testing.T, n int) (*network.Network, []types.CV) {
	t.Helper()
...
func TestListCV(t *testing.T) {
	net, objs := networkWithCVObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListCV(), args)
			require.NoError(t, err)
			var resp types.QueryAllCVResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
