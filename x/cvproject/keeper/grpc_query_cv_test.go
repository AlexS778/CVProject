...
func TestCVQueryPaginated(t *testing.T) {
	keeper, ctx := keepertest.CvprojectKeeper(t)
	wctx := sdk.WrapSDKContext(ctx)
	msgs := createNCV(keeper, ctx, 5)

	request := func(next []byte, offset, limit uint64, total bool) *types.QueryAllCVRequest {
		return &types.QueryAllCVRequest{
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(msgs); i += step {
			resp, err := keeper.CVAll(wctx, request(nil, uint64(i), uint64(step), false))
			require.NoError(t, err)
			require.LessOrEqual(t, len(resp.CV), step)
			require.Subset(t,
				nullify.Fill(msgs),
				nullify.Fill(resp.CV),
			)
		}
	})
	t.Run("ByKey", func(t *testing.T) {
		...
		var next []byte
		for i := 0; i < len(msgs); i += step {
			resp, err := keeper.CVAll(wctx, request(next, 0, uint64(step), false))
			require.NoError(t, err)
			require.LessOrEqual(t, len(resp.CV), step)
			require.Subset(t,
				nullify.Fill(msgs),
	...
	t.Run("Total", func(t *testing.T) {
		resp, err := keeper.CVAll(wctx, request(nil, 0, 0, true))
		require.NoError(t, err)
